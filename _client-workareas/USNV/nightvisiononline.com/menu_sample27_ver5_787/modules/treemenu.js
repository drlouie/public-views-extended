/*
   Milonic DHTML Menu - Tree Menu Module Version 1.27 - January 31 2007
...
function treeMenuDisplay($m,_show){                      // This function hides or shows the menu
	...
	_m[$m][7]=_show                                      // Set the menus visibility property to Show or Hide 
	_m[$m].treemenu=1;
	$Y($m,_show); 
}
...
//	alert(NWID)
	...
	else                                                   // Display this child menu
	{
		...
		_TI.ttop=_PiGP[0]+_PiGP[2]-_TI[27]+s_ // set temporary top property for the 
		...
		$E(_TI.child,_TI.ttop,_TI.tleft)         // set the position of the menu to open
		...
		if(_TI.children)
		{
			for(_tm=0;_tm<_TI.children.length;_tm++)
			{
				...
				$E($c("menu"+$m),_m[$m].ttop,_m[$m].tleft) // set the position of this items menu
...
function resetParents(_ti)
{
	...
	while(_m[$m].parentItem)
	{
		...
		$E(_m[$m].parentItem,_n,_n,_m[$m].HGT-s_)
		//$z($m)
...
{	
	...
	$E(_m[_TI.childN].parentItem,_n,_n,_m[_TI.childN].OHGT)
}
