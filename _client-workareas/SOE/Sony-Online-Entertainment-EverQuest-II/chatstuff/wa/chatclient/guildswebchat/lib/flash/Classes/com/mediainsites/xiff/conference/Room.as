...
import com.mediainsites.xiff.core.XMPPConnection;
import com.mediainsites.xiff.data.*;
import com.mediainsites.xiff.data.muc.*;
import com.mediainsites.xiff.data.forms.FormExtension;
 ...
 *
 * @availability Flash Player 7
 * @see com.mediainsites.xiff.data.Message
 */
[Event("groupMessage")]
 ...
 *
 * @see com.mediainsites.xiff.data.forms.FormExtension
 * @see com.mediainsites.xiff.data.forms.FormField
 * @see configure
 * @availability Flash Player 7
 */
[Event("configureForm")]
 ...
 * @toc-sort 1
 */
class com.mediainsites.xiff.conference.Room
{
	 ...
	 * @availability Flash Player 7
	 * @see com.mediainsites.xiff.core.XMPPConnection
	 */
	public function setConnection( connection:XMPPConnection ):Void
	{
	 	...
	 	myConnection.removeEventListener( "disconnect", this );

		myConnection = connection;
	 	
	 	myConnection.addEventListener( "message", this );
	 	myConnection.addEventListener( "presence", this );
	 	myConnection.addEventListener( "disconnect", this );
	}
	...
	private function handleEvent( eventObj ):Void
	{
		switch( eventObj.type )
		{
			case "message":
				...
				if( isFromThisRoom( msg.from ) && msg.type == Message.GROUPCHAT_TYPE ) {
				...
				else if( msg.to == getUserJID() && msg.type == Message.CHAT_TYPE ) {
                ...
                // Could be an decline to a previous invite
                    ...
                    if (muc.type == MUCUserExtension.DECLINE_TYPE) {
                        dispatchEvent({
                            type:"declined", 
                            target:this, 
                            from: muc.from,
			...
			case "presence":
				...
				} else if( isFromThisRoom( presence.from ) ) {
					if( !isActive() )
						active = true;
	...
	private function updateRoomRoster( aPresence:Presence ):Void
	{
		...
		if (item != undefined && item.jid.split("/")[0] == myConnection.getBareJID()) {
			// TODO notify listeners of room status change
			myAffiliation = item.affiliation;
			myRole = item.role;
		}

		for( var i=0; i < rosterItems.getLength(); i++ ) {
			if( rosterItems.getItemAt( i ).nickname == userNickname ) {
				...
				if( aPresence.type == Presence.UNAVAILABLE_TYPE ) {
					...
					var eventObj:Object = {target:this, type:"userDeparture", nickname:userNickname};
				...
				if (item != undefined) {
					rosterItems.editField( i, "affiliation", item.affiliation );
					rosterItems.editField( i, "role", item.role );
					rosterItems.editField( i, "show", aPresence.show != null ? aPresence.show : Presence.SHOW_NORMAL );
				}
				return;
			}
		}
		...
		if( aPresence.type != Presence.UNAVAILABLE_TYPE ) {
			// Notify listeners that a user has joined, as long as it isn't us
			if( userNickname != nickname ) {
				var eventObj:Object = {target:this, type:"userJoin", nickname:userNickname};
				dispatchEvent( eventObj );
			}
			
			addToRoomRoster( userNickname, aPresence.show != null ? aPresence.show : Presence.SHOW_NORMAL, item.affiliation, item.role );
		}
	}
	
	private function addToRoomRoster( nickname:String, show:String, affiliation:String, role:String ):Void
	{
		rosterItems.addItem( {nickname:nickname, show:show, affiliation:affiliation, role:role} );
	}
	
	private function isFromThisRoom( sender:String ):Boolean
	{
		// Checks to see that sender is this room
		if( sender.split( "/" )[0].toLowerCase() == getRoomJID().toLowerCase() ) {
			return true;
		}
		else {
			return false;
