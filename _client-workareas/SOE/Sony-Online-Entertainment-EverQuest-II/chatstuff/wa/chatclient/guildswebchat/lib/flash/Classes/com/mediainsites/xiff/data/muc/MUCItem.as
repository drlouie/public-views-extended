 ...
 * @toc-sort 1/2
 */
class com.mediainsites.xiff.data.muc.MUCItem extends XMLStanza implements ISerializable
{
	...
	{
		if (!exists(role)) {
			trace("Warning: required MUC item attributes 'role' missing");
			return false;
		}

		if (!exists(affiliation)) {
			trace("Warning: required MUC item attributes 'affiliation' missing");
			return false;
		}
